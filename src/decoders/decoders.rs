...
impl Decoder {
    ...
    pub fn decode_bencoded_integer(&mut self) -> Vec<i64> {
        ...
        let mut integers: Vec<(usize, i64, Option<usize>)> = Vec::new();
        ...
        for (i, char) in self.encoded_value.chars().enumerate() {
            ...
            if previous_char == "i" && char.to_string().to_lowercase() == "e" && modified {
                if let Some(last_index) = integers.len().checked_sub(1) {
                    if let Some(data) = integers.get_mut(last_index) {
